...
def read_redis_list():#list
    ...
    while total_num>0:
        ...
        with r.pipeline(transaction=False) as p:
            ...
            data = p.execute()[0]
            print("data",type(data),len(data),data)
            print("jiayuan_item:items当前数量",total_num)
            for  item in data:
                sql_data={}
                a = json.loads(item)
    ...
    #             print("work_study",len(a['work_study'].split(';')),a['work_study'].split(';'))
    ...
    #             print("person_dict",type(person_dict),person_dict)
                ...
                sql_data['age_info']=a['age_info']
                sql_data['sex']=a['sex']
                sql_data['img_urls']=','.join(a['img_urls'])
                sql_data['age']=a['age']
                sql_data['municipal']=a['municipal']
                sql_data['address']=a['address']
                sql_data['person_id']=a['person_id']
                sql_data['url']=a['url']
                sql_data['nike_name']=a['url']
                sql_data['province']=a['province']
                sql_data['image_dir']=a['image_dir']
                sql_data['introduce_oneself']=a['introduce_oneself']
                
                sql_data['interest_label']=a['interest_label']#爱好
                sql_data['personality_label']=a['personality_label']#个人标签
                ...
                '''
                sql_data['housework'] = a['housework'][0]
                sql_data['household_assignment'] = a['pet'][0]
                sql_data['pet'] = a['housework'][1]
                sql_data['about_pets'] = a['pet'][1]
                
                '''
                处理person_info一定有值
                '''
                person_info = parse1(sub_str(a['person_info']).split(';'))
                ...
                处理择偶要求find_mate一定有值
                '''
                find_mate = parse2(';'.join(sub_str(a['find_mate']).split('：')).replace(';;',';'))
               ...
               处理 生活方式不一定有
                '''
                if(len(sub_str(a['life_style']).split(';'))<=2):#说明该项目未填写
                    ...
                    life_style = parse2(sub_str(a['life_style'].replace(';;',';')))
                ...
                if(len(sub_str(a['life_style']).split(';'))<=2):#说明该项目未填写
                    ...
                    economic_strength = parse2(sub_str(a['economic_strength'].replace(';;',';')))
                ...
                处理工作学习
                '''
                if(len(sub_str(a['work_study']).split(';'))<=5):#说明该项目未填写
                    ...
                    work_study = parse2(sub_str(a['work_study'].replace(';;',';')))
                ...
                处理婚姻观念
                '''
                if(len(sub_str(a['marriage_concep']).split(';'))<=25):#说明该项目未填写
                    sql_data['position'] = '你很想了解她的工作学习吧，邀请她补充'
                    sql_data['company'] = '你很想了解她的工作学习吧，邀请她补充'
                    sql_data['company_type'] = '你很想了解她的工作学习吧，邀请她补充'
                    sql_data['welfare'] = '你很想了解她的工作学习吧，邀请她补充'
                    ...
                    marriage_family = parse2(sub_str(a['marriage_concep'].replace(';;',';')))
